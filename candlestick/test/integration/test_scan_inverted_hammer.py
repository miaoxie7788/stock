...
from candlestick import scan_benchmark, scan_inverted_hammer
...
def test_stock(stock_code, stock_params, path="data/asx_stock/csv"):
    ...
    eval_dict = scan_inverted_hammer(df, **stock_params)
...
if __name__ == "__main__":
    ...
    #
    # # sz_stocks = ["300655.SZ", "300122.SZ", "002007.SZ"]
    #
    # result_df = pd.DataFrame([test_stock(stock, params_dict, "data/sz_stock/csv") for stock in sz_stocks])
    #
    # result_df.to_csv("sz_scan_inverted_hammer_results.csv", index=False, header=True)
    ...
    result_df.to_csv("asx_scan_inverted_hammer_results3.csv", index=False, header=True)
